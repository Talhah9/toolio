// Toolio — Auth page (login / register)

const Auth = ({ navigate, initialMode = 'login' }) => {
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <button className="btn btn-secondary btn-lg btn-block" onClick={() => { setLoading(true); setTimeout(() => navigate('dashboard'), 500); }}>
        ...
        <form onSubmit={submit}>
          ...
          <div className="field">
            ...
            <div style={{ position: 'relative' }}>
              <input className="input" type={showPwd ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required style={{ paddingRight: 40 }} />
              <button type="button" onClick={() => setShowPwd(!showPwd)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--fg-4)', display: 'flex' }}>
        ...
        <p className="muted" style={{ fontSize: 12, textAlign: 'center', marginTop: 24 }}>
          {mode === 'register' ? 'En vous inscrivant, vous acceptez nos ' : ''}
          {mode === 'register' && <a style={{ textDecoration: 'underline', cursor: 'pointer' }}>conditions</a>}
          {mode === 'register' && ' et notre '}
          {mode === 'register' && <a style={{ textDecoration: 'underline', cursor: 'pointer' }}>politique de confidentialité</a>}
          {mode === 'register' && '.'}
        </p>
      </div>
    </div>
